...
class GrayscaleSkeleton:
    ...
    def skeletonize(self, image):
        ...
        for curr_threshold in image_thresholds:
            ...
            mask = (image < curr_threshold)
            curr_thresh_image = image.copy()
            curr_thresh_image[mask] = 0

            curr_binary_image = curr_thresh_image.astype(bool).astype(np.uint8) * 255
            imwrite(skeleton_images_path + "binary_" + str(curr_threshold) + ".png", curr_binary_image)

            curr_sum_image = (prev_binary_image + curr_binary_image // 255)
            curr_skeleton_image = self.thin_pixels(curr_sum_image)
            imwrite(skeleton_images_path + "skeleton_" + str(curr_threshold) + ".png", curr_skeleton_image.astype(np.uint8) * 255)
    ...
    def thin_pixels(self, image):
        neighbors = nd.convolve((image > 0).astype(int), [[1, 1, 1], [1, 0, 1], [1, 1, 1]], mode='constant', cval=0.0)
...
def add_zero_mat(image):
    ...
    image = np.insert(image, num_rows, np.zeros(num_cols, dtype=int), 0)
    image = np.insert(image, 0, np.zeros(num_cols, dtype=int), 0)

    num_rows, num_cols = image.shape

    image = np.insert(image, num_cols, np.zeros(num_rows, dtype=int), 1)
    image = np.insert(image, 0, np.zeros(num_rows, dtype=int), 1)
...
if __name__ == '__main__':
    ...
    skeleton_image.save(output_filename)
